...
class TagManager(models.Manager):
      def get_Tag_list(self): #返回文章标签列表, 每个标签以及对应的文章数目
          ...
          for i  in range(len(tags)):
              tag_list.append([])
          for i  in range(len(tags)):
              temp = Tag.objects.get(name = tags[i]) #当前标签
              posts = temp.article_set.all() #当前标签下的文章
              tag_list[i].append(tags[i].name)#如果这里加了.name.encode，错误信息会不同哟
              #tag_list[i].append(tags[i])
              tag_list[i].append(len(posts))
          return tag_list
      def get_Article_onTag(self):#返回tag，以及每个tag对应的文章（多个）
          tags=Tag.objects.all()#获取所有标签
          post_tag_article=[]
          #TAlist=[]
          for i in range(len(tags)):
              post_tag_article.append([])
              #TAlist.append([])
          for i in range(len(tags)):
              temp=Tag.objects.get(name=tags[i]) #当前标签
              tempposts=temp.article_set.all() #当前标签下的文章
              post_tag_article[i].append(temp)
              post_tag_article[i].append(tempposts)
          
          # for i in range(len(tags)):
          #     templist=[]
          #     templist[i].append(tags[i].name)
          #     templist[i].append(post_tag_article[i])
          #     TAlist.append(templist[i])
          return post_tag_article

class Tag(models.Model):
      name = models.CharField(max_length=20,blank=True)
      #creat_time = models.DateTimeField(auto_now_add=True)
      #这里有未解决问题，当加入时间后，在主页调用sidebar会有反向解析错误NoReverseMatch
      #就是说往url传了个字典过去。
      ...
      @models.permalink
      def get_name(self): #后来加的，试试看
        tagname=self.name.all()
        return tagname
